...
AtDCore.prototype.getLang = function(key, defaultk) {
  if (this.i18n[key] == undefined) return defaultk;
...
AtDCore.prototype.findSuggestion = function(element) {
  ...
  errorDescription["description"] = this.getSurrogatePart(
    metaInfo,
    "description"
  );
  errorDescription["coveredtext"] = this.getSurrogatePart(
    metaInfo,
    "coveredtext"
  );
...
AtDCore.prototype.markMyWords = function() {
  ...
  for (
    var suggestionIndex = this.suggestions.length - 1;
    suggestionIndex >= 0;
    suggestionIndex--
  ) {
...
AtDCore.prototype.getSurrogatePart = function(surrogateString, part) {
  ...
  console.log(
    "No part '" + part + "' found in surrogateString: " + surrogateString
  );
  return null;
};
...
AtDCore.prototype._getPlainText = function(removeCursor) {
  var plainText = tinyMCE.activeEditor
  ...
  this.map(this.findSpans(node).reverse(), function(n) {
    if (
      ...
      if (n.innerHTML == "&nbsp;") {
        ...
        var textIsRelevant = coveredText
          ? parent.getSurrogatePart(surrogate, "coveredtext") == coveredText
          : true;
        if (
          textIsRelevant &&
          (surrogate && parent.getSurrogatePart(surrogate, "id") == ruleId)
        ) {
          parent.removeParent(n);
          count++;
        }
...
AtDCore.prototype.hasErrorMessage = function(xmlr) {
  return (
    xmlr != undefined && xmlr.getElementsByTagName("message").item(0) != null
  );
};
  ...
  tinymce.create("tinymce.plugins.AfterTheDeadlinePlugin", {
    ...
    initAtDCore: function(editor, plugin) {
      ...
      core.findSpans = function(parent) {
        if (parent == undefined) return editor.dom.select("span");
        else return editor.dom.select("span", parent);
      };
    ...
    init: function(ed, url) {
      ...
      editor.addCommand("mceWritingImprovementTool", function(languageCode) {
        ...
        plugin.sendRequest("", textContent, languageCode, function(
          data,
          request,
          jqXHR
        ) {
          ...
          if (results.suggestions.length == 0) {
            var lang = plugin.editor.getParam(
              "languagetool_i18n_current_lang"
            )();
            var noErrorsText =
              plugin.editor.getParam("languagetool_i18n_no_errors")[lang] ||
          ...
          if (results.incompleteResults) {
            $("#feedbackErrorMessage").html(
              "<div id='severeError'>These results may be incomplete due to a server timeout.</div>"
            );
            t._trackEvent(
              "CheckError",
              "ErrorWithException",
      ...
      editor.onInit.add(function() {
        ...
        if (editor.settings.content_css !== false) {
          editor.dom.loadCSS(
            editor.getParam("languagetool_css_url", url + "/css/content.css?v4")
          );
        }
      });
      ...
      editor.onPaste.add(function(editor, ev) {
        ...
        var marketingText =
          "NEU: Unter <a href='https://languagetoolplus.com/'>languagetoolplus.com</a> bieten wir eine Premium-Version an, die noch mehr Fehler erkennt.";
        var randThreshold = 0.3;
        var langCode = $("#lang").val();
        if (document.cookie && document.cookie.indexOf("showltplus=") === -1) {
          var rand = Math.random();
          document.cookie =
            "showltplus=" + rand.toFixed(2) + ";max-age=2628000"; // one month
          if (
            rand < randThreshold &&
            (langCode === "de-DE" ||
              langCode === "de-AT" ||
            ...
            $("#feedbackErrorMessage").html(
              "<div id='survey'>" + marketingText + "</div>"
            );
          }
        } else if (document.cookie) {
          var rand = parseFloat(
            document.cookie.match(/showltplus=(\d\.\d\d)/)[1]
          );
          if (
            rand < randThreshold &&
            (langCode === "de-DE" ||
      ...
      editor.onPreProcess.add(function(sender, object) {
        ...
        each(dom.select("span", object.node).reverse(), function(n) {
          if (
            n &&
            (dom.hasClass(n, "hiddenGrammarError") ||
              dom.hasClass(n, "hiddenSpellError") ||
              dom.hasClass(n, "hiddenSuggestion") ||
              dom.hasClass(n, "mceItemHidden") ||
              (dom.getAttrib(n, "class") == "" &&
                dom.getAttrib(n, "style") == "" &&
                dom.getAttrib(n, "id") == "" &&
                !dom.hasClass(n, "Apple-style-span") &&
                dom.getAttrib(n, "mce_name") == ""))
          ) {
            dom.remove(n, 1);
          }
        });
    ...
    _showMenu: function(ed, e) {
      ...
      if (ed.core.isMarkedNode(e.target)) {
        ...
        if (errorDescription == undefined) {
          m
            .add({
              title: plugin.editor.getLang(
                "AtD.menu_title_no_suggestions",
                "No suggestions"
              ),
        ...
        var explainText =
          plugin.editor.getParam("languagetool_i18n_explain")[lang] ||
          "Explain...";
        var ignoreThisText =
          plugin.editor.getParam("languagetool_i18n_ignore_once")[lang] ||
          "Ignore this type of error";
        var ruleExamples = "Examples...";
        if (plugin.editor.getParam("languagetool_i18n_rule_examples")) {
          ruleExamples =
            plugin.editor.getParam("languagetool_i18n_rule_examples")[lang] ||
            "Examples...";
        }
        var noRuleExamples = "Sorry, no examples found for this rule.";
        if (plugin.editor.getParam("languagetool_i18n_rule_no_examples")) {
          noRuleExamples =
            plugin.editor.getParam("languagetool_i18n_rule_no_examples")[
              lang
        ...
        if (plugin.editor.getParam("languagetool_i18n_rule_implementation")) {
          ruleImplementation =
            plugin.editor.getParam("languagetool_i18n_rule_implementation")[
              lang
            ] || "Rule implementation...";
        }
        ...
        if (plugin.editor.getParam("languagetool_i18n_suggest_word")) {
          suggestWord =
            plugin.editor.getParam("languagetool_i18n_suggest_word")[lang] ||
            "Suggest word for dictionary...";
        }
        var suggestWordUrl;
        if (plugin.editor.getParam("languagetool_i18n_suggest_word_url")) {
          suggestWordUrl = plugin.editor.getParam(
        ...
        if (
          ...
          errorDescription["moreinfo"] != null
        ) {
          (function(url) {
            m.add({
              title: explainText,
              onclick: function() {
                window.open(url, "_errorExplain");
              }
            });
        ...
        if (!isSpellingRule) {
          m.add({
            ...
            onclick: function() {
              ...
              var surrogate = e.target.getAttribute(
                plugin.editor.core.surrogateAttribute
              );
          ...
          if (plugin.editor.getParam("languagetool_i18n_ignore_all")) {
            ignoreThisKindOfErrorText =
              plugin.editor.getParam("languagetool_i18n_ignore_all")[lang] ||
          ...
          m.add({
            ...
            onclick: function() {
              ...
              var ruleId = plugin.editor.core.getSurrogatePart(surrogate, "id");
              var coveredText = plugin.editor.core.getSurrogatePart(
                surrogate,
                "coveredtext"
              );
        ...
        if (suggestWord && suggestWordUrl && isSpellingRule) {
          var newUrl = suggestWordUrl.replace(
            /{word}/,
            encodeURIComponent(errorDescription["coveredtext"])
          );
          (function(url) {
            m.add({
              title: suggestWord,
              onclick: function() {
                window.open(newUrl, "_suggestWord");
              }
            });
        ...
        var ruleUrl =
          ...
          "?";
        if (
          errorDescription["subid"] &&
          errorDescription["subid"] !== "null" &&
          errorDescription["subid"] !== "undefined"
        ) {
          ruleUrl += "subId=" + encodeURI(errorDescription["subid"]) + "&";
        }
        ruleUrl += "lang=" + encodeURI(langCode);
        var isLTServer =
          window.location.href.indexOf("languagetool.org") !== -1 ||
          window.location.href.indexOf("languagetool.localhost") !== -1; // will only work for lt.org because
        if (
          isLTServer &&
          errorDescription["id"].indexOf("MORFOLOGIK_") !== 0 &&
          errorDescription["id"] !== "HUNSPELL_NO_SUGGEST_RULE"
        ) {
          // no examples available for spell checking rules
          m.addSeparator();
          m.add({
            title: ruleExamples,
            onclick: function() {
              ...
              jQuery
                .getJSON(
                  "/online-check/tiny_mce/plugins/atd-tinymce/server/rule-proxy.php?lang=" +
                    encodeURI(langCode) +
                    "&ruleId=" +
                    errorDescription["id"],
                  function(data) {
                    ...
                    $.each(data["results"], function(key, val) {
                      if (
                        ...
                        if (
                          val.corrections &&
                          val.corrections.length === 1 &&
                          val.corrections[0] !== ""
                        ) {
                          var escapedCorr = $("<div/>")
                            .text(val.corrections[0])
                            .html();
                    ...
                    ruleHtml +=
                      "<p><a target='_lt_rule_details' href='" +
                      ruleUrl +
                      "'>" +
                      ruleImplementation +
                      "</a></p>";
                ...
                .fail(function(e) {
                  var $dialog = $("#dialog");
                  $dialog.html(
                    "Sorry, could not get rules. Server returned error code " +
                      e.status +
                      "."
                  );
                  $dialog.dialog("open");
              ...
              error: function(jqXHR, textStatus, errorThrown) {
                ...
                if (data.length > maxTextLength) {
                  ...
                  errorText =
                    ...
                    data.length +
                    " characters). This server accepts up to " +
                    maxTextLength +
                    " characters.";
                }
                $("#feedbackErrorMessage").html(
                  "<div id='severeError'>" + errorText + "</div>"
                );
